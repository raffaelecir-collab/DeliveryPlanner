...
var REGOLE_DEFAULT = [
  ...
  { chiave: 'pesoRicavo', valore: '0.2', tipo: 'numero', descrizione: 'Peso base del ricavo (€) di un intervento nella scelta di quali interventi assegnare: fa preferire gli interventi più redditizi, senza mai bloccare il riempimento delle ore disponibili del turno (il target resta un indicatore, non un limite che ferma le assegnazioni). Questo peso viene rafforzato automaticamente (fino a 5 volte) quando la produzione della squadra è ancora lontana dal proprio target giornaliero, e torna al valore base avvicinandosi/superando il target. Aumenta questo valore base se vuoi che il ricavo pesi di più in generale, azzeralo per ignorarlo del tutto.' },
  { chiave: 'pesoCompetenzaSpecifica', valore: '150', tipo: 'numero', descrizione: 'Priorità data agli interventi che richiedono esplicitamente una competenza posseduta dalla squadra, rispetto a quelli generici (competenza vuota, assegnabili a qualsiasi squadra): un tecnico specializzato riceve prima il lavoro della propria specializzazione, usando quello generico solo come riempitivo quando non ce n\'è (più) a sufficienza. Non ha effetto sulle squadre senza competenze specifiche impostate.' }
];
